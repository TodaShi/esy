...
package org.ry8.external.androidquery.service;
...
import org.ry8.external.androidquery.util.AQUtility;
...
public class MarketService{
    ...
    public TextView setting_updatevalue;
...
//			url += "&force=true";
//		}
        String url = getHost() + "/mobile/upgrade.html?appId=" + appId + "&t="
                + new Date();
        if (force) {
            url += "&force=true";
        }
    ...
    protected void callback(String url, JSONObject jo, AjaxStatus status){
		...
		*/
        ...
        String latestVer = jo.optString("version", "0");
        int latestCode = jo.optInt("code", 0);
        Log.i("AQuery", "version" + getVersion() + "->" + latestVer + ":"
                + getVersionCode() + "->" + latestCode);
        AQUtility.debug("version", getVersion() + "->" + latestVer + ":"
                + getVersionCode() + "->" + latestCode);
        AQUtility.debug("outdated", outdated(latestVer, latestCode));

        String showlatestVer = latestVer;

        if (force || outdated(latestVer, latestCode)) {
            if (isShow) {
                showUpdateDialog(jo);
            } else {
                if (setting_updatevalue != null && !isShow && !"".equals(latestVer)) {
                    setting_updatevalue.setText(getVersion());
                }
            }
        } else {
            if(context!=null) {
                ToastView toast = new ToastView(context, "已经是最新版本" + showlatestVer);
...
//		dialog.setMessage(Html.fromHtml(patchBody(body), null, handler));
                        ...
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog,
                                ...
                                UpdateManager mUpdateManager = new UpdateManager(
                                        act);//
                                mUpdateManager.setApkUrl(marketUrl);
        ...
        dialog.setMessage(Html.fromHtml(patchBody(body)));
